...
def push_training(sender, instance, created, **kwargs):
    devices = GCMDevice.objects.all()
    if len(devices) > 0:
        devices.send_message(SiteConfiguration.get_solo().new_training_message if created else SiteConfiguration.get_solo().update_training_message , extra={"event":TrainingSerializer(instance, many=False).data})

# def push_training_step(sender, instance, created, **kwargs):
#     devices = GCMDevice.objects.all()
#     if len(devices) > 0:
#         devices.send_message(SiteConfiguration.get_solo().new_training_step_message if created else SiteConfiguration.get_solo().update_training_step_message , extra={"event":TrainingStepSerializer(instance, many=False).data})

def push_post(sender, instance, created, **kwargs):
    devices = GCMDevice.objects.all()
    if len(devices) > 0:
        devices.send_message(SiteConfiguration.get_solo().new_post_message if created else SiteConfiguration.get_solo().update_post_message , extra={"post":PostSerializer(instance, many=False).data})

def push_media(sender, instance, created, **kwargs):
    devices = GCMDevice.objects.filter()
    if len(devices) > 0:
        devices.send_message(SiteConfiguration.get_solo().new_media_message if created else SiteConfiguration.get_solo().update_media_message , extra={"media":MediaSerializer(instance, many=False).data})
